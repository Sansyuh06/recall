...
Usage:
    memoriagrain diff --against last-deploy
"""
...
from memoriagrain.judge import _judge_with_heuristic
from memoriagrain.store.base import Atom, Store

logger = logging.getLogger(__name__)

SNAPSHOT_PATH = Path(".memoriagrain") / "last_deploy.json"
...
def diff_against_last_deploy(store: Store) -> list[DiffResult]:
    ...
    if snapshot is None:
        logger.info("No deploy snapshot found. Run 'memoriagrain diff --save' first.")
        return []
...
def _check_invalidation(
    ...
    if isinstance(old_tools, list):
        for tool in old_tools:
            ...
            if re.search(r"\b" + re.escape(tool_name) + r"\b", atom_text):
                return DiffResult(
                    memory_id=atom.id,
                    grain="atom",
    ...
    if old_prompt and len(old_prompt) > 50:
        ...
        from memoriagrain.store.base import Atom as PseudoAtom
